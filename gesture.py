...
class Controller:
    ...
    mapped_functions = {
        "Cursor movement" : moveMouse,
        "Double Click": doubleClick, 
        "Right Click": rightClick,
        "Left Click": leftClick,
        "Multiple Item Selection": moveMouseLeftClick,
        "Scroll Vertically": scrollVertical,
        "Scroll Horizantally": scrollHorizontal,
        "Volume Control": changesystemvolume,
        "Brightness Control": changesystembrightness
    }
    

    # Localize cursor by Dampening
    ...
    def handle_controls(gesture, hand_result):
        ...
        if gesture == Gest.V_GEST:
            print(Controller.mapped_functions[Controller.settings['two_finger_move']])
            Controller.mapped_functions[Controller.settings['two_finger_move']](x, y)

        elif gesture == Gest.FIST:
            print(Controller.mapped_functions[Controller.settings['fold_hand_move']])
            Controller.mapped_functions[Controller.settings['fold_hand_move']](x, y)

        elif gesture == Gest.MID and Controller.flag:
            print(Controller.mapped_functions[Controller.settings['middle_finger']])
            Controller.mapped_functions[Controller.settings['middle_finger']]()

        elif gesture == Gest.INDEX and Controller.flag:
            print(Controller.mapped_functions[Controller.settings['index_finger']])
            Controller.mapped_functions[Controller.settings['index_finger']]()

        elif gesture == Gest.TWO_FINGER_CLOSED and Controller.flag:
            Controller.mapped_functions[Controller.settings['two_finger_close']]()
            ...
            left_pinch_hor = Controller.mapped_functions[Controller.settings['left_pinch_hor']]
            left_pinch_ver = Controller.mapped_functions[Controller.settings['left_pinch_ver']]
            Controller.pinch_control(hand_result, left_pinch_hor, left_pinch_ver)
            ...
            right_pinch_hor = Controller.mapped_functions[Controller.settings['right_pinch_hor']]
            right_pinch_ver = Controller.mapped_functions[Controller.settings['right_pinch_ver']]
            Controller.pinch_control(hand_result, right_pinch_hor, right_pinch_ver)
        ...
        with mp_hands.Hands(max_num_hands=2, min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
            while GestureController.cap.isOpened() and GestureController.gc_mode:
                ...
                if results.multi_hand_landmarks:
                    ...
                    if self.settings['lines_drawn'] == "true":
                        for hand_landmarks in results.multi_hand_landmarks:
                            mp_drawing.draw_landmarks(image, hand_landmarks, mp_hands.HAND_CONNECTIONS)
                else:
                    Controller.prev_hand = None
                cv2.imshow('Gesture Controller', image)
                if cv2.waitKey(5) & 0xFF == 13 and not GestureController.stop_thread:
                    break
